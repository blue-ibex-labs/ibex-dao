...
export async function getServerSideProps() {
  const res = await fetch(
    `https://ibex-dao.vercel.app/api/assets?policy=9cb921b32bfe214a739ed824f3f2da4e16c535a5448253d2951cc732`
  );

  const data = await res.json();
  const assetID = data?.assets?.map(
    async (val) =>
      await fetch(`https://ibex-dao.vercel.app/api/asset?id=${val?.asset}`)
  );
